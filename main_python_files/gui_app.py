...
class AquaCleanGUI(ctk.CTk):
    def __init__(self):
        ...
        self.geometry("1400x700") # Made it wider for 3 panels
        ...
        # 🟢 Load a Font for clear labels
        try:
            # Change "arial.ttf" to your system's font path if needed
            self.font = ImageFont.truetype("arial.ttf", 22) 
        except:
            self.font = ImageFont.load_default()

        # --- UI LAYOUT (Sidebar and Main Container) ---
        self.sidebar = ctk.CTkFrame(self, width=200)
        self.sidebar.pack(side="left", fill="y", padx=10, pady=10)

        self.btn_select = ctk.CTkButton(self.sidebar, text="Select Image", command=self.load_image)
        self.btn_select.pack(pady=20, padx=10)

        # 🟢 Button 2: Store to Database (New)
        ...
        # 2. Create Image Panels (Row 1) - Set text="" so no text shows in middle
        ...
        # Configure weights so they resize correctly
        self.img_container.grid_columnconfigure((0, 1, 2), weight=1)
        self.img_container.grid_rowconfigure(1, weight=1) # Row 1 takes the space

        # 🟢 New Dummy Function for Database Button
    def save_to_db(self):
        print("💾 Database storage triggered! (Function ready for DB logic)")
    ...
    def load_image(self):
        ...
        # 2. Draw Detections with Accuracy %
        det_img = orig_img.copy() # Drawing detections on the original image as requested
        ...
        # Refresh UI
        self.display_image(orig_img, self.panel_orig)
        self.display_image(enh_img, self.panel_enh)
        self.display_image(det_img, self.panel_det)
